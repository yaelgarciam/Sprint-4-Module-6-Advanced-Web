...
-- Set SKILL_ID and ACTIVE on all topics (idempotent UPDATEs)
UPDATE TOPICS SET SKILL_ID = 1, ACTIVE = 1 WHERE ID IN (1,2,3,20)   AND (SKILL_ID IS NULL OR SKILL_ID != 1);
UPDATE TOPICS SET SKILL_ID = 2, ACTIVE = 1 WHERE ID IN (4,5,6,7)    AND (SKILL_ID IS NULL OR SKILL_ID != 2);
UPDATE TOPICS SET SKILL_ID = 3, ACTIVE = 1 WHERE ID IN (8,9,10,11)  AND (SKILL_ID IS NULL OR SKILL_ID != 3);
UPDATE TOPICS SET SKILL_ID = 4, ACTIVE = 1 WHERE ID IN (12,13,14)   AND (SKILL_ID IS NULL OR SKILL_ID != 4);
UPDATE TOPICS SET SKILL_ID = 5, ACTIVE = 1 WHERE ID IN (15,16)      AND (SKILL_ID IS NULL OR SKILL_ID != 5);
UPDATE TOPICS SET SKILL_ID = 6, ACTIVE = 1 WHERE ID IN (17,18,19)   AND (SKILL_ID IS NULL OR SKILL_ID != 6);
-- Topic 16 (Seguridad en Redes) inactive for testing
UPDATE TOPICS SET ACTIVE = 0 WHERE ID = 16;

-- =====================  CATALOGUE — LEARNING_PATHS  =========
-- Enroll some students; leave gaps for "not enrolled" testing
...
MERGE INTO LEARNING_PATHS tgt USING (SELECT  6 AS ID, 'A00835003' AS SID,  8 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT  7 AS ID, 'A00835003' AS SID,  9 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT  8 AS ID, 'A00835004' AS SID, 12 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT  9 AS ID, 'A00835005' AS SID, 15 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT 10 AS ID, 'A00835005' AS SID, 16 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT 11 AS ID, 'A00835006' AS SID,  1 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT 12 AS ID, 'A00835006' AS SID,  5 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT 13 AS ID, 'A00835007' AS SID, 17 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT 14 AS ID, 'A00835008' AS SID, 10 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
MERGE INTO LEARNING_PATHS tgt USING (SELECT 15 AS ID, 'A00835009' AS SID, 13 AS TID FROM DUAL) src ON (tgt.ID=src.ID) WHEN NOT MATCHED THEN INSERT(ID,STUDENT_ID,TOPIC_ID) VALUES(src.ID,src.SID,src.TID);
